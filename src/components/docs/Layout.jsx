...
function flat(item, res = []) {
    item?.map(oneDoc => {
        ...
        if (oneDoc.hasOwnProperty("links")) {
...
export default function Layout({children, title, tableOfContents}) {
    const navigation = useLanguageQuery()[0]?.lang === "en" ? en_navigation : zh_navigation
    let router = useRouter()
    let allLinks = flat(navigation);
    let linkIndex = allLinks.findIndex((link) => link.href === router.pathname)
    let previousPage = allLinks[linkIndex - 1]
    let nextPage = allLinks[linkIndex + 1]
    let section = allLinks.find((one) =>
        one.href === router.pathname
    )
    return (
        <>
